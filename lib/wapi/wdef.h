...
#define S_16B		16				// 16 bytes buffer size for typical commands
#define S_32B		16 + S_16B		// 16 bytes buffer size for typical set calls + command space
#define S_272B		256	+ S_16B		// 256 bytes max string buffer size for typical get calls + command space
#define S_2KB		2*1024			// 2kB receive buffer size (typical MTU is 1400 bytes) for some calls
#define GET_COUNT 	50				// number of attempts at reading data before giving up on not receiving
									// expected set
//
